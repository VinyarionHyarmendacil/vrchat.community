...
If you don't have the credentials available at the time of creating the `VRChat` instance, you can lazily re-authenticate when needed.
This is useful for scenarios where you want to handle authentication errors gracefully, such as prompting the user for their credentials when needed.
```ts twoslash
import { VRChat } from "vrchat";
