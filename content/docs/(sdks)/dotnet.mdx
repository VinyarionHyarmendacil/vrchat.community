...
For accounts with 2FA enabled, you can provide a TOTP secret for seamless login:
...
The following example showcases how you can work with the raw generated library yourself. This is not the recommended way but can be used for very specific use cases.

<Callout type="warn" title="Advanced Usage">
This approach gives you direct access to the generated API client, but requires more manual configuration and error handling.
</Callout>
...
// We can't use emails here because HTTP header parser complains
config.UserAgent = "ExampleProgram/0.0.1 mydiscordusername"; 

// Create a client to hold all our cookies
...
try
{
    ...
    {
        if (currentUserResp.Data.RequiresTwoFactorAuth.Contains("emailOtp")) 
        {
        ...
        else if(currentUserResp.Data.RequiresTwoFactorAuth.Contains("totp"))
        {
            ...
            // authApi.VerifyRecoveryCode(new TwoFactorAuthCode("12345678"));
            
            authApi.Verify2FA(new TwoFactorAuthCode("123456"));
        }
    }

    // We can now get our CurrentUser
...
{
    // Catch any exceptions and write to console
...
### Cookies with Generated API

Cookies can be displayed after logging in using:

```csharp
ApiClient.CookieContainer.GetAllCookies()
    .ToList()
...
```csharp
config.DefaultHeaders.Add("Cookie", 
    "auth=[AUTH_COOKIE_HERE]; twoFactorAuth=[TWO_FACTOR_AUTH_COOKIE_HERE]");
```

## Documentation

For detailed API documentation, see the [docs](https://github.com/vrchatapi/vrchatapi-csharp/tree/main/docs/) directory in the repository.
